...
def main():
    ...
    # Call the count_marriages function to calculate and print the total number of marriages each person has had.
...
def print_death_age(people_dict):
    ...
    for people in people_dict:
        birth_year = people_dict[people][BIRTH_YEAR_INDEX]
        death_year = people_dict[people][DEATH_YEAR_INDEX]
        age = death_year - birth_year
        print(f"{people_dict[people][NAME_INDEX]} {age} ({birth_year}-{death_year})")
...
def count_genders(people_dict):
    ...
    for people in people_dict:
        gender = people_dict[people][GENDER_INDEX]
        if gender == "M":
            total_male += 1
        else:
            total_female += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    print("Marriages")
    for marriages_key in marriages_dict:
        husband_key = marriages_dict[marriages_key][HUSBAND_KEY_INDEX]
        wife_key = marriages_dict[marriages_key][WIFE_KEY_INDEX]
        husband_name = people_dict[husband_key][NAME_INDEX]
        wife_name = people_dict[wife_key][NAME_INDEX]
        husband_age_at_marriage = marriages_dict[marriages_key][WEDDING_YEAR_INDEX] - people_dict[husband_key][BIRTH_YEAR_INDEX]
        wife_age_at_marriage = marriages_dict[marriages_key][WEDDING_YEAR_INDEX] - people_dict[wife_key][BIRTH_YEAR_INDEX]
        marriage_year = marriages_dict[marriages_key][WEDDING_YEAR_INDEX]

        print(f"{husband_name} {husband_age_at_marriage} > {marriage_year} < {wife_name} {wife_age_at_marriage}")

def count_marriages(marriages_dict, people_dict):
    SUMMATION_INDEX = 4

    for people in people_dict:
        people_dict[people].append(0)

    for marriages_key in marriages_dict:
        husband_key = marriages_dict[marriages_key][HUSBAND_KEY_INDEX]
        wife_key = marriages_dict[marriages_key][WIFE_KEY_INDEX]
        people_dict[husband_key][SUMMATION_INDEX] += 1
        people_dict[wife_key][SUMMATION_INDEX] += 1

    print()
    print("Total marriages by person")
    for people in people_dict:

        total_number_marriages = people_dict[people][SUMMATION_INDEX]
        name = people_dict[people][NAME_INDEX]
        print(f"{name} {total_number_marriages}")
